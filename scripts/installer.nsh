; IClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting IClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers IClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if IClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old IClaw was installed
  ...
  ; "Can't modify IClaw's files" retry loop -> "IClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "IClaw 无法关闭" dialog.
...
; "IClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with IClaw's
