...
# Post-installation script for IClaw on Linux
...
# Create symbolic link for IClaw app binary
if [ -x /opt/IClaw/clawx ]; then
    ln -sf /opt/IClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/IClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/IClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/IClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/IClaw/resources/apparmor-profile'
...
echo "IClaw has been installed successfully."
